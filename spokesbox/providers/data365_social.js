    ...
    const req = https.request(options, res => {
      ...
      res.on('end', () => {
        if (res.statusCode < 200 || res.statusCode >= 300) {
          ...
          return reject(new Error(`Data365 HTTP ${res.statusCode}: ${preview}`));
        }
        const ct = res.headers['content-type'] || '';
        if (!ct.includes('application/json') && !ct.includes('text/json')) {
          const preview = data.slice(0, 120).replace(/\n/g, ' ');
          return reject(new Error(`Data365 non-JSON response (${ct}): ${preview}`));
        }
        try { resolve(JSON.parse(data)); }
        catch (e) { reject(new Error(`Data365 JSON parse error: ${e.message} — body: ${data.slice(0,80)}`)); }
      });
    });
    req.on('error', reject);
